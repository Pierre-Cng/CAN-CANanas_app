...
class Oscilloscope:
    ...
    def set_canvas(self):
        ...
        self.canvas.get_tk_widget().grid(row=0, column=1, sticky="nsew", padx=10, pady=10, rowspan=2)
        ...
        if self.running:
            ...
            for signal in self.clicked_signals:
                if signal in self.signals:
                    if len(self.signals[signal]['x'])>30:
                        self.ax.plot(self.signals[signal]['x'][-30:-1], self.signals[signal]['y'][-30:-1], label=signal)
                    else:
